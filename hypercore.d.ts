declare module "hypercore" {
    ...
    interface HypercoreValueEncoding<T> {
        encode(buf: T): Buffer;
        decode(buf: Buffer): T;
    }
    ...
    interface HypercoreGetOptions<T> {
        wait?: boolean;
        timeout?: number;
        valueEncoding?: 'json' | 'utf-8' | 'binary' | HypercoreValueEncoding<T>;
    }

    interface HypercoreDownloadRange {
        start: number;
        end?: number;
        linear?: boolean;
    }

    interface HypercoreSignatureIndex {
        index?: number,
        signature?: Buffer
    }

    interface HypercoreNode {
        index: number;
        size: number;
        hash: Buffer;
    }

    interface HypercoreStreamOptions {
        start?: number;
        end?: number;
        snapshot?: boolean;
        tail?: boolean;
    ...
    interface HypercoreReplicateOptions {
        live?: boolean;
        download?: boolean;
        encrypt?: boolean;
    }

    interface Hypercore<T> extends EventEmitter {
        writable: boolean;
        readable: boolean;
        key: Buffer | null;
